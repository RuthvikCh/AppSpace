...
include("conn.php");
  $sql1 = " SELECT DISTINCT * FROM apptable WHERE genre = '$genre' LIMIT 12 ;";
  $select = mysqli_query($conn, $sql1);
  $num_rows = mysqli_num_rows($select);
  $rows = mysqli_fetch_array($select, MYSQLI_ASSOC);
 
  if ($num_rows > 0) {
      echo" <h3 class='gsans' style='margin: 1.2rem; margin-bottom:0;'>".$genre ."</h3>  <div class='searchout'>";
    while ($rows = mysqli_fetch_array($select, MYSQLI_ASSOC)) {
        $aname=$rows['app_name'];
        echo "
    <a href='dload.php?app_name=".$aname ."'> 
      <div class='outitem'>
